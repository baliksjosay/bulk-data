...
/**
 * Provides SMS OTP-based MFA challenges and verification.
 */
@Injectable()
export class SmsOtpMfaProvider implements MfaProvider {
  readonly method = MfaMethod.SMS_OTP;
  ...
  /**
   * Indicates whether SMS OTP MFA is available.
   *
   * This provider is available only when the user has a phone number.
   */
  isAvailable(): boolean {
    return true;
  }
  ...
  /**
   * Creates an SMS OTP MFA challenge and sends the OTP through the
  ...
  async createChallenge(
    ...
    if (!user.phoneNumber) {
      throw new BadRequestException(
        'User does not have a phone number for SMS MFA',
      );
    }
  ...
  /**
   * Verifies the submitted SMS OTP against the stored challenge hash.
   */
